...
\fixme{Need a reference to the first figure and a caption for it. Anne}
...
\begin{dunefigure}[Dual-phase readout]{fig:dualphase-principle}{Illustration of the electric fields in the amplification region of a dual-phase LArTPC. The simulated field lines in dark blue indicate the paths followed by the drifting charges (without diffusion).}
 \includegraphics[width=.8\textwidth]{dualphase-principle}  
\end{dunefigure}
...
\fixme{Add in appropriate subsections for the pieces that the Charge Readout Planes interfaces with. These initial ones may not be right.}
