...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Charge Readout Planes (\dword{crp}) Overview}
...
By applying voltages across the two electrodes of the \dword{lem}, an \efield region (up to \SI{35}{kV/cm}) is defined in the holes, allowing to achieve a gain exceeding \num{20} after the initial phase of  charging-up the \dword{lem} dielectric material. The nominal operation conditions correspond to a again of \num{20} after charging-up. Electrons transiting these high \efield regions in the holes trigger Townsend multiplication in the pure argon gas.
...
\begin{dunefigure}
[View of a complete \dword{crp} module of \SI{9}{m$^{2}$}]
...
During cooling, INVAR's dimensions remain nearly unchanged whereas the G10 frame and \dwords{lem}-anodes contract in similar ways. Thermal decoupling allows a lateral sliding of the G10 frame, without changing the altitude. \fixme{altitude? maybe `level' is better?}
...
For the vertical position, %the use of the
 a suspension system and its motorization on three points provides an accurate measurement for each \dword{crp} of the order of \SI{0.1}{mm} thanks to the accuracy of the steps motors and encoders. 
 Prior to these measurements the known absolute position of the \dword{crp} will be obtained by survey of suspensions and anchor points on the \dword{crp}. There will be also the possibility to get relative measurements of the \dword{crp} position with respect to the \lar level using capacitive level meters put on the sides of the  external \dword{crp} of the detection plane. 
 \fixme{prev pgraph needs work but I don't understand it well enough to improve it; some accuracy vs precision issues too}
 
The  measurements of the capacitance between the \dword{lem} and the extraction grid, which depends on the height of the liquid above the grid, could also give a relative measurement  of the \dword{crp}'s vertical position with respect to the liquid surface.

For the lateral measurement of the \dword{crp}, capacitive devices called \textit{distance meters} made of two parallel plates %will be used for 
provide the relative alignment of  \dword{crp} lateral position. This device gives an accuracy of the order of \SI{0.1}{mm} on the  inter-\dword{crp} distance. 
With these capacitive measurements, no contact is required and four devices per \dword{crp} side are embedded in the G10 frame side as can be seen on Figure~\ref{fig:distancemeter}.

\begin{dunefigure}[oView of the distance meter plates on the sides of a \dword{crp}]{fig:distance meter}
{View of the distance meter plates on the sides of a \dword{crp}.}
\includegraphics[width=0.85\textwidth]{distancemeter.png}
\end{dunefigure}
\fixme{Fig \ref{fig:distance meter} could use more explanation}
...
Three suspension \fdth{}s are arranged in an equilateral triangle whose barycenter coincides with that of the \dword{crp}; they suspend the \dword{crp} at the required position and precisely adjust the \dword{crp} level with respect to the \lar surface.
...
%Indicate \dword{qa} acceptance tests (performed by the \dword{crp} production centers)
...
\fixme{This section not filled in (anne). Needed?}
...
The \dword{crp} assembly production activity will span over two years 
\fixme{span 2 yrs or > 2yrs?} prior to \dword{crp} installation underground at \surf. The \dwords{crp} will be integrated in at least two different production sites and stored locally in temporary storage boxes before being shipped to the ITF delivery facility. The \dword{crp} assembly will be pipelined with the \dword{lem} and anode production and testing during these two production years with an initial  shift of three months. This is intended to constitute a large enough buffer between the \dwords{lem} and anodes to start the \dword{crp} assembly activities.

\fixme{Put here the details of the assembly procedures copied from the example of \dword{pddp} in the clean room 185 with also some cad pictures and pictures of the real tooling and elements. The assembly movie can be also put  as external link. (not anne; how to handle?)}
...
connecting the \fdth flange from the top of the cryostat to distribution boxes located on the \dword{crp} (Figure \ref{fig:CRP_DB}). Each cable (\num{20}\,AWG, \SI{50}{\ohm} KAPTON insulated coaxial cable from ACCU-GLASS~\footnote{ACCU-GLASS\texttrademark{}, \url{http://www.accu-glass.com/}}) can sustain \SI{30}{kV} in air and has SHV plugs on each end in order to facilitate connections during the \dword{crp} installation.

There are six cables used for the \dword{lem} top electrodes and \num{36} cables for the bottom ones. Each of the coaxial cables for the \dword{lem} top electrodes is connected to a single distribution box. The later contains a PCB that %allows to 
...
\begin{dunefigure}
[High voltage distribution box for \dword{lem} top electrodes before filling]
{fig:LEM_DB} 
{High voltage distribution box for \dword{lem} top electrodes before filling with ARATHANE glue. }
...
The production of the \dwords{crp} will be performed before the installation, and the \dwords{crp} will be stored in temporary storage boxes at the production centers. During installation the already produced \dwords{crp} will be moved to the transportation boxes in order to be shipped to the ITF and then moved underground. Given the installation rate of 8 \dwords{crp}/month a set of \num{30} transportation boxes will ensure a sufficient turnover among the production sites and the installation site.
\fixme{I can't make out when they leave the production site to go to the ITF, and when they leave ITF for installation at the 4850. It appears to be staggered.}
...
Once the three \dwords{crp} are ready to be lifted, three persons are needed to manipulate the manual winches on the top of the cryostat for a few hours. This activity is expected to occur during the \num{1.3}-week period just before the cabling activity for the connection to the flanges.
...
The transportation boxes are sized for the shafts and underground handling at \surf{}.
...
The \dword{crp} calibration relies on a pulsing system that performs charge injection in the strips via \SI{1}{pF} capacitors. The pulse distribution system and related cabling, and the boards with the charge-injection capacitors are integrated  with the \dword{crp} at the time of the \dword{crp} assembly. At this time, this system is connected via flat cables to the instrumentation flange. A pulsing distribution system external to the cryostat and  provided by the \dword{cisc} consortium ensures the driving of the charge injection system. 
\fixme{drives the system?} The information from the pulse calibration is then complemented 
\fixme{combined?} with the one from the \dword{fe} electronics calibration and the one from the analysis of the reconstructed charged tracks in order to extract the overall calibration constants per channel.
...
The \dword{crp} installation and operation does not present particular safety issues apart from working at height during the \dword{crp} installation for the connection of the cabling to the \dwords{sftchimney} and the instrumentation and \dword{hv} \fdth{}s.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\subsection{\dword{crp} Consortium Organization}
