...
For optimization purposes, one \dword{dpmod} HVDB will connect \num{11} stages.  This enables  one \tpcheight tall module to be covered by fifteen \num{11} stage HVDB and one \num{10} stage HVDB at the bottom to make the final connection.  


Figure~\ref{fig:dp-hvdb} shows one HVDB board and  Figure~\ref{fig:dune-dp-fc-all}.d. shows the connection of one row of HVDB along the entire height of the \fc as installed in \dword{pddp}. The  \dptpcwdth (W) $\times$ \dptpclen (L) ground plane consists of \num{80} unit planes (each \SI{3}{\m} $\times$ \SI{3}{\m}).  
