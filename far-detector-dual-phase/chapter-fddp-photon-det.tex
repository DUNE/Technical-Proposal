...
\includegraphics[width=0.75\textwidth]{dppd_2_2b}
\end{dunefigure}
...
For the type of HV cables between HV splitters and feedthroughs, the HTC 50-3-2 \cite{htc-50-3-2} have been chosen as baseline. The HTC 50-3-2 has a similar attenuation length compared to the RG-303/U \cite{rg303} which will be used inside the cryostat, but for a factor of \num{8} to \num{10} lower cost. These cables will be attached on one side directly to the HV splitter and will have an SHV connector on the other end.
...
\includegraphics[width=0.45\textwidth]{dppd_3_2}
\end{dunefigure}
...
\fixme{
As a starting point toward understanding whether the dual-phase photon detector technical design meets the experiment's physics requirements, several physics events are being produced using LArSoft fast optical simulation tools, and performance plots are being generated. For the 3rd iteration of the TP draft in April, we plan to include plots for the following three event topologies:

1) Low-energy nue CC interactions in LAr generated with the Marley event generator in LArSoft. The energies will be representative of SN neutrino burst events, eg either 10 MeV mono-energetic neutrinos, or a neutrino flux following a Fermi-Dirac distribution with T=3.5 MeV temperature.

2) High-energy nue CC interactions in LAr generated with the Genie event generator in LArSoft. The energies will be representative of DUNE beam events, eg either 3 GeV mono-energetic neutrinos, or following a characteristic DUNE neutrino flux.

3) Proton decay events in LAr generated with the Genie event generator in LArSoft. For a representative decay mode, such as p -> nubar K+ or p -> e+ pi0. 

For each event topology above, we plan to include two plots:

a) Histogram of detected light yields per event (in PEs) for events generated throughout the entire TPC active volume

b) Efficiency-like curve with fraction of events above a certain PE detection threshold, as a function of PE detection threshold 

These optical simulation results represent our current state-of-the-art. They include realistic models for the primary scintillation production yields in LAr, for Rayleigh scattering in LAr, for detector optical properties such as field cage reflectivity and cathode transparency, for the areal density of PMTs underneath the cathode, and for the quantum efficiency of the TPB-coated PMTs. 

On the other hand, these simulations do not yet include the full dual-phase far detector geometry (but are rather based on a ProtoDUNE-DP geometry with the same PMT density as there one proposed here for the far detector), secondary scintillation light emission in gaseous argon (a nuisance for event t0 determination), light absorption in LAr, electronics effects, reconstruction effects, and background contributions coming from Ar-39 decays. More realistic simulation results including also these effects will be produced on the TDR timescale.
}
