...
The light maps generation takes roughly three days of computing time to generate the maps for \dword{pddp}, even though only %1/8$^{th}$ 
...
The \dword{pds} trigger will be flexible to fulfill the different physics requirements explained before. 
\fixme{suggest: The \dword{pds} trigger design is flexible so as to fulfill the different physics requirements explained before.} %%%% anne
The light readout \dword{fe} board will be in charge of the \dword{pds} trigger generation. The trigger will be decided based on the coincidence of several \dword{pmt} signals over a threshold during a time window. The number of \dwords{pmt} that contribute to the trigger, the signal threshold and the length of the coincidence time window will be programmable on-line to be able to adapt to different physics cases.
...
\item Calibration: The \dword{pds} will participate in the Global Calibration Task Force and will provide handles to allow global monitoring of the \dword{pmt} performance.
\fixme{will participate or is participating?}
...
The \dwords{pmt} are individually wrapped with different wrapping materials. The wrappings will have special openings to enable the basic electronics tests at the \dword{itf}. \fixme{different wrapping materials?}
...
\item The wrapping materials and techniques are studied with one fully assembled light readout unit. The handling, transportation and installation scenarios are carefully studied and the transportation box design is validated. The transport box and \dword{pmt} wrapping must warrant darkness. \fixme{must ensure complete darkness??}
...
Upon receipt at \dword{itf}, the \dwords{pmt} go through verification measurements in order to discard possible damages during transportation. 
\fixme{discard any damaged items?}  Gain versus voltage and dark current values are compared with those obtained before transportation.

The \dword{tpb} coating is also performed at the \dword{itf}. The first few samples undergo microscopic examination and surface uniformity tests, and the coating procedure is validated. The production \dwords{pmt} are randomly sampled for basic coating \dword{qa}.
\fixme{QC or QA?}
...
The main activities to be developed by the \dual \dword{pd} consortium during the next \num{16} months are focused to complete the \dword{tdr} of the \dual \dword{pds}. The main high-level milestones are detailed in Table~\ref{tab:dppd_t_12_4}
