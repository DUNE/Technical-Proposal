...
\begin{dunefigure}[DP front-End \dword{daq} fragment]{fig:daq-readout-buffering-baseline}
  {Illustration of data (solid arrows) and trigger (dashed) flow for
    two DP front-end \dword{daq} fragments. 
...
DP-specific implementation of the nominal, generic \dword{daq} front-end
...
The CRO data is delivered to the \dword{daq} BOW computers with lossless
...
intervenes. 
Thus each CRO requires a corresponding \SI{10}{\Gbps} \dword{nic} in the receiving
BOW computer.
...
The current generation of \dword{felix} boards have been tested to with a
...
Given their expected data rates, that \dword{felix} board must ingest about
\SI{25}{\GB/\s}.
...
With today's technology, individual \dwords{ssd} can write at about 2.5 GB/s. 
Up to four \dwords{ssd} have been placed on a 16 lane PCIe v3 board and have
...
streamed to a two modern \dwords{ssd}. 
The BOW computers must also have sufficient RAM to hold
pre-\dword{snb}-trigger data of about 10 seconds.
...
The CRO (compressed) data is buffered on the \dword{felix} host computer ram
