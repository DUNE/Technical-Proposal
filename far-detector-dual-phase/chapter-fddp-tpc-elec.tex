...
The \dword{lro} electronics system, illustrated in the Figure~\ref{fig:dpele-lrosystem-sketch},  collects and digitizes the signals from the \dword{pds}, which consists of \dword{tpb}-coated \num{8}\,in \dwords{pmt} (Hamamatsu\footnote{Hamamatsu\texttrademark R5912-02-mod, \url{http://www.hamamatsu.com/}.} R5912-02-mod) located beneath the TPC cathode. The \dword{lro} electronics %should 
...
\begin{dunefigure}[Signal \fdth chimney cold flange]{fig:dpele-sft-cold-pcb}
...
\begin{dunefigure}[Instrumented \dshort{utca} crate from the \dword{wa105}]{fig:dpele-311-utca-image}
...
%The chimneys are delivered with the cold and warm flanges already mounted and and leak tested. %by one or several participating institutions.  
...
The \dwords{sftchimney} are commissioned as a first step. This consists of evacuating and then filling them with nitrogen gas at slight overpressure. % with respect to the atmosphere. % pressure. 
...
\item{\textbf{Damage to electronics due to \dword{hv} discharges or other causes}: %reasons}: 
...
must be set low enough such that no significant damage could happen to the flanges. }
...
consists of seven participating institutions from France (\num{3}), Japan (\num{3}), and the USA (\num{1}). The consortium leader is %Dario Autiero (
from IPNL, France, and the technical leader is %Takuya Hasegawa (
...
%The composition of the consortium along with the information for each institutional representative is provided in Table~\ref{tab:dpele-consortium-people}.

% REMOVING NAMES OF PEOPLE JUNE 2108; AH REMOVING TABLE: \dual TPC electronics consortium current participants
