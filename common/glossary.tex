...
\newduneabbrev{nd}{ND}{near detector}{Refers to the detector(s) or more
  generally the experimental site at \fnal.}

\newduneabbrev{fd}{FD}{far detector}{Refers to the detector or more
  generally the experimental site at \surf in
  Lead, SD.}
...
\newduneabbrev{tpc}{TPC}{time projection chamber}{The portion of each
  DUNE \gls{submodule} that records ionization electrons
...
\newduneabbrevs{apa}{APA}{anode plane assembly}{anode plane assemblies}{One unit the \single
  detector module containing the elements sensitive to activity in the \lar. 
...
\newduneabbrev{rce}{RCE}{reconfigurable computing element}{One of four
  nodes in a \gls{cob} that consists of ARM CPU and FPGA resources. ??}
...
\newduneabbrev{utca}{$\mu$TCA}{Micro Telecommunications Computing Architecture}{A computer architecture. Vertical card-edge connectors, Amphenol ICC,  \url{https://www.amphenol-icc.com/product-series/micro-tca-card-edge.html}.} %?? needs work
...
\newduneabbrev{fpga}{FPGA}{field programmable gate array}{An
  integrated circuit technology that allows the hardware to be reconfigured
...
\newduneword{datafrag}{data fragment}{A block of data read
  out from a single \gls{daqfrag} that covers a contiguous period of time
  as requested by a \gls{trigcommand}. continuous???}
...
\newduneword{coldata}{COLDATA}{COLDATA needs def ???}
\newduneword{larasic}{LArASIC}{LArASIC needs def ???}
...
\newduneword{protodune}{ProtoDUNE}{Either of two DUNE prototype detectors operated in
  a CERN beam test. 
  One prototypes \gls{sp} and the other \gls{dp} technology.}
...
\newduneword{rawevent}{DAQ event block}{The unit of data output by the
  ...
  time period and a subset of the detector channels.   continuous??}
...
\newduneabbrev{snb}{SNB}{supernova neutrino burst}{A prompt and brief
  ...
  Can also refer to a trigger command type that may be due to an SNB,
...
\newduneabbrev{cdr}{CDR}{conceptual design report}{A formal project
  document required by funding agencies that describes the experiment
...
\newduneabbrev{lbl}{LBL}{long-baseline}{Refers to the distance between
  ...
  The ``long'' designation is an approximate and relative distinction. For DUNE, this distance is approximately \SI{1300}{km}.}
...
\newduneabbrev{tp}{TP}{technical proposal}{An intermediate
milestone on the path to a full \gls{tdr}.}
...
\newduneabbrev{ce}{CE}{cold electronics}{Generally, describes readout electronics that operate at cryogenic temperatures.}
...
\newduneword{lem}{LEM}{Large Electron Multiplier} %%?? should be abbrev
...
\newduneabbrev{wr}{WR}{White Rabbit}{A timing system} % ?? needs more info
...
\newduneabbrev{dss}{DSS}{detector support system}{A system of supporting a \single detector within the cryostat.}
...
\newduneabbrev{tco}{TCO}{temporary construction opening}{An opening in the cryostat utilized during construction and installation.}

\newduneabbrev{uit}{UIT}{underground installation team}{An organizational unit responsible for installation in the \surf underground area.}
...
\newduneword{rtd}{RTD}{?? Email sent to Glenn}
