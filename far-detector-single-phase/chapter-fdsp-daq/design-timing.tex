...
from Fermilab containing predictions of the GPS-time of the spills. 
Experience from MINOS and NOvA shows that this can provide beam
triggering with high reliability, although the system outlined here
contains an extra layer of redundancy in this process. 
Several stages of narrowing down the window in which the beam arrives
will be done, aiming for an accuracy of better than 10\% of a readout
time (which is nominally somewhat more than two times the max drift
time or \spreadout for the \dword{sp} and \dpreadout for the
\dword{dp} \dword{detmodule}) at the time the data are selected from
the DAQ buffers. 
