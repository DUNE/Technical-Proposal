...
\metainfo{Georgia Karagiorgi and Dave Newbold. 2 Pages - largely generic but some highlighting of SP-specifics.  Focus on describing to HEP but non-DAQ expert.  Include how design is resilient in the face of potential uncertainties such as excess noise or the need to reduce drift HV (just two examples, maybe there are more).}
...
module, including data from the \dfirst{pds}, \dfirst{tpc},
...
relatively little challenge to the \dword{daq} system, due to the
...
the \dword{tpc} and \dword{pds}. On the other hand, the low activity
...
rate fluctuations, the approach followed by the \dword{daq}
...
An overview of the overall DUNE \dword{fd} \dword{daq} system is illustrated in
Fig.~\ref{fig:daq-overview}. 
Figure~\ref{fig:daq-overview} illustrates the main \dword{daq} subsystem
components as well as the data flow and the exchange of trigger and
monitoring messages between them.
As shown, it is applicable to both \dword{tpc} and \dword{pds} readout, except
that in the case of the \dword{pds} the \dword{daq} scope begins to the right of
the \dfirst{fe} readout. 
%\fixme{Describe figure here.}

\fixme{Can we include the other sub-detectors (or at least PDS) in this figure for clarity, with dotted lines to demark the DAQ scope?  The SP TPC is a special case...  It would also be useful to indicate how other modules fit in.  The figure includes a bunch of terms that are not defined in the caption - suggest to simplify the figure and introduce some of the sub-structure later?}
...
\fixme{Suggest : A requirements at the DAQ output is that the bandwidth available to ship data to off-site data centers for offline analysis is limited to \SI{100}{\Gbps}.}
...
\fixme{Can we choose a less alarming phrase?!}

\fixme{Moved Tabs 7.1 and 7.2 closer to referencing text}

\fixme{Tab 7.2 is really unclear.  Need to clarify the physical origin, trigger and readout type for each one. Assumptions also include quite a bit of jargon.}
...
\fixme{Table 7.2 - 10s for extended events is out of date. What are we saying ?}

A \dword{trigdecision} in the DUNE FD concerns:
\begin{enumerate}
...
includes all channels.  \fixme{We're saying we readout everything for both}
Acquiring an \dword{snb} dump inflicts no subsequent dead time for
nominal data taking.
...
\fixme{This paragraph seems to say, in a roundabout way, that we read out all channels for all types of trigger.  Update statement about SNB readout window.  And let's change ``dump'' for something else.}
