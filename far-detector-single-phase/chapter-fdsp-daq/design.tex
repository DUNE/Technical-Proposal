...
APA Level \dword{trigcandidates}, Module (10 ktonne) Level triggers, and External
...
low-energy \dword{trigcandidates} exceeds a threshold for the number of such events
...
just one more ``external'' trigger input.
	...
	APA Level \dword{trigcandidates} will be generated within each server of the \dwords{fec}.  The trigger decision will be based on the number of adjacent wires hit
...
pileup from $^{39}$Ar and noise.
	...
	At the APA Level, two kinds of local \dword{trigcandidates} can be
...
across many APAs is consistent with a supernova.
	...
	The Module Level takes as input both APA Level \dword{trigcandidates} (both
...
weighting scheme to individual APA Level \dword{trigcandidates}---higher-energy
\dword{trigcandidates} receiving higher weights. Thus, for example, two APA Level
\dword{trigcandidates} consistent with 10 MeV interactions in 10 seconds might be
...
All low-energy \dword{trigcandidates} are also passed upwards to the External
Trigger level, which can integrate the APA level trigger across all 10 ktonne
modules and determine if enough APAs have \dword{trigcandidates} that a supernova
...
trigger was created. A log of all APA Level \dword{trigcandidates} will also be
