...
\dword{detmodule} level, as well as combined over the full four \dwords{detmodule}. 
...
APA) and placed in a \dword{ringbuffer} while \dwords{trigdecision}
are made. 
...
\fixme{Clarify what is meant by ``main data''.  ``trigger decision'' is confusing - sometimes it refers to a decision, sometimes the positive outcome of a decision.  No need to mention ``trigger command'' at this stage - we can just say what happens to the data without mentioning implementation.  Unclear what the secondary DAQ buffer is for.  Event building undefined.}
...
The trigger primitives are then sent to the \dword{detmodule} level and 
...
readout commands to an appropriate \dwords{data selector}. 
...
\SI{10}{\s}. 
The \dword{snb} \dword{trigcommand} is issued to the \dword{oob} which
...
\metainfo{Josh Klein \& J.J. Russel \& Brett Viren \& DP Expert?}
...
\metainfo{Josh Klein \& Brett Viren}
...
\metainfo{Giovanna Miotto \& Jingbo Wang}
