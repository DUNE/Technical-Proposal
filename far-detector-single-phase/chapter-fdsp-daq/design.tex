...
APA Level trigger candidates, Module (10 ktonne) Level triggers, and External
...
All low-energy trigger candidates are also passed upwards to the External
Trigger level, which can integrate the APA level trigger across all 10 ktonne
modules and determine if enough APAs have trigger candidates that a supernova
...
sent  to all FELIX hosts instructing them to save a 5.4~ms readout window will
written for all wires across all APAs (providing the start and end times to the
FELIX hosts).  Thus, a DUNE ``event'' is 5.4~ms of data from all wires in
