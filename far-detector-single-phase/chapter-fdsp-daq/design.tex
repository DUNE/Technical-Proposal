...
APA Level \dwords{trigcandidate}, Module (10 ktonne) Level triggers, and External
...
low-energy \dwords{trigcandidate} exceeds a threshold for the number of such events
...
just one more ``external'' trigger input.
	...
	APA Level \dwords{trigcandidate} will be generated within each server of the \dwords{fec}.  The trigger decision will be based on the number of adjacent wires hit
...
across many APAs is consistent with a supernova.
	...
	The Module Level takes as input both APA Level \dwords{trigcandidate} (both
...
weighting scheme to individual APA Level \dwords{trigcandidate}---higher-energy
\dwords{trigcandidate} receiving higher weights. Thus, for example, two APA Level
\dwords{trigcandidate} consistent with 10 MeV interactions in 10 seconds might be
...
All low-energy \dwords{trigcandidate} are also passed upwards to the External
Trigger level, which can integrate the APA level trigger across all 10 ktonne
modules and determine if enough APAs have \dwords{trigcandidate} that a supernova
...
trigger was created. A log of all APA Level \dwords{trigcandidate} will also be
