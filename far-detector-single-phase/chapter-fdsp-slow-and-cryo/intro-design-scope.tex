...
a comprehensive monitoring for all \dword{detmodule} components as well as for the \lar quality and behavior, both being crucial
to guaranty a good quality of the data. Beyond passive monitoring, \dword{cisc} also provides a control system for some of the detector components. 
...
that their designs do not induce large field distortions. \dshort{cfd}
...
\dunelifetime (current official DUNE run time).  Since it is uncommon for any device
...
is designed to be robust enough to support a large number of variables and a broad range of
monitoring and archiving rates, and %has the capability 
to interface
...
\begin{dunetable}
[Important design requirements on the \single \dword{cisc} system design]
...
while the \dword{cisc} consortium provides the cost 
\fixme{``provides'' the cost?} and supplements the labor as
...
test facility, \dword{cisc} will be responsible from design to commissioning in
the \dwords{fd}.

From the slow controls side, \dword{cisc} will provide control and monitoring of
...
% 6787 (HV), and 6790 (DAQ). The \dword{cisc} interface documents with facility,
