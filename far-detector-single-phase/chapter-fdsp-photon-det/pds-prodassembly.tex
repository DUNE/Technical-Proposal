...
Although the individual cell dimensions may differ, the basic design of the ARAPUCA-based \dword{pd} modules for the first  \dword{spmod} will be similar to that of the two prototypes produced for \dword{pdsp}. Here we describe the production and assembly envisaged based on that experience.
...
To produce the full-size \dword{pdsp}  dip-coated light guide bars, the production methods initially developed at MIT for 20'' dip-coated light guide bars were scaled up for a facility at \dword{fnal}. For \dword{pdsp} production four steps will remain essentially the same:
...
A picture of the oven used for annealing, the fume hood used for mixing the coating, the vessel used for dipping, and the dark box used for measuring attenuation lengths for the production of dip-coated light guides for \dword{pdsp} is shown in Figure~\ref{fig:dipprod}.  These same production methods will also be used for the  \dwords{spmod}, although they will need to be scaled up so that multiple bars can be dipped and scanned at the same time.  Additionally, multiple production sites will be built to produce dip-coated light guide bars for the  \dwords{spmod}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\dword{apa} Frame Mounting Structure and Module Securing}	
...
were \SI{108.0}{mm}$\times$\SI{19.2}{mm} wide (see Figure~\ref{fig:pds-pd-mounting}(top))\footnote{These dimensions are expected to increase by about 20\% in the next round of \dword{apa} design revisions, allowing for larger \dword{pd} modules.}.  
...
\begin{dunetable}[Relative shrinkage of \dword{pd} components and \dword{apa} frame with mitigations]
...
FEA modeling of the \dword{pd} support structure was conducted to study static deflection 
