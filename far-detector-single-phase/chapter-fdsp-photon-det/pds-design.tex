...
The core modular element of the PD system are the large area {\it photon collectors} that convert incident \SI{127}{nm} scintillation photons into photons in the visible range (>\SI{400}{nm}), which in turn are converted to an electrical signal by compact silicon photomultipliers (SiPM) {\it photon sensors}. At the time of the Technical Proposal there are three photon collector options under consideration; Figure~\ref{fig:3dtpc-pd} shows how they are incorporated into the TPC anode plane assembly by an identical mounting scheme. In the following we summarize the design and development status for each photon collector option. 
...
Photon trapping is achieved by using a smart wavelength-shifting technique and the technology of the dichroic shortpass optical filters, which are multilayer thin films with the property of being highly transparent to photons with a wavelength below a tunable cut-off while being almost perfectly reflective to photons with wavelength above the cut-off.  Such a filter deposited with one or two different wavelength-shifters is the entrance window to a squat box with internal surfaces covered by highly reflective acrylic foils 
...
\begin{dunefigure}[ARAPUCA test at the Brazilian Synchrotron Light Laboratory (LNLS)]{LNLS_test}
...
Each ARAPUCA array is composed of sixteen cells. Each cell is an ARAPUCA box with dimensions of 8 cm $\times$ 10 cm and has 12 SiPMs (or 6 SIPMs in some cases) installed on the bottom side of the cell (6 mm $\times$ 6 mm each), as illustrated in Figure \ref{fig:arpk}.
...
%delimiting one side of the LArTPC volume. 
...
\item{begin}
?	A 48 SiPM summing board for SENSL 4x4 arrays. This is an active ganging board of 48 3mm x 3mm SiPMs from SenSL C series. This detector has been fully characterized and tested at TallBo with an alpha source (Am241) in March 2017. For the TallBo test the SiPM arrays were coated with 100ug/cm2 of TPB and showed an efficiency of 13%.
?	A 12 SENSL (6x6 mm C series) summing board that was used by the IU group in their light bars during the TallBo run of Oct-Nov 2017.
?	We have tested Hamamatsu MPPCs (S13360-6050PE) at 25C, -70C and 77K. A passive ganging of 2, 4, 8 and 12 MPPCs were characterized and showed single PE separation.
?	A passive gang of 4 SENSL (6x6 mm C series) for ARAPUCAs during the TallBo run of Oct-Nov 2017.
?	A passive gang of 6 and 12 MPPCs for ARAPUCA ProtoDUNE design
?	A 72 combination of passive and active ganging using 6mm x 6mm MPPCs with a lower noise OP Amp. This design combines 12 active branches into the low noise OpAmp. Each branch has 6 MPPCs in parallel passive configuration.
\item{end}
