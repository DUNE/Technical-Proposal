...
\item Accommodate the variation in the fee through locations and variation in the flange angle due to installation tolerances and the loading on the warm structure.  
\item Accommodate the shrinkage of the detector and DSS from ambient temperatures to LAr temperatures.
\item Minimize the gaps that develop between APAs during cool down less than 13mm (WHAT IS THE CORRECT VALUE?)  A proposed design uses beams that are 6.4m long, which support typically 3 APAs.  This design results in a zero gap developing between the three APAs on the same beam but a 17mm gap opening up between adjacent APAs that are on separate beam.  Roughly there will be a 17mm gap between every 3rd and 4th APA.
