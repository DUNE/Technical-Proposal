...
installation. On the surface TC is responsible for coordinating with
the LBNF logistics effort and acting as the interface to LBNF
logistics for planning the transport of all equipment underground. TC
will receive equipment from the consortia and plan the transport the
...
transport underground. Given possible delays in shipping and weather a
...
also provide the detector support system along with the switchyard
...
\begin{itemize}
  \item {\bf Transport Buffer:} Storage capacity for one month material in the vicinity of
...
failure, for example, repair, return to source or discard.
  \item {\bf Visitor Support:} Consortia will likely send staff to the
    ITF to participate in the integration, testing and installation of
    the consortia detector components. Long-term Sanford Lab visitors
    may choose to live and part-time work in the area of the ITF
    rather than near Sanford Lab for personal or family reasons. The
    ITF may be able provide space, computer access, assistance
    personnel and other infrastructure support for DUNE visitors.
...
steel membrane and the insulation. The logistics for the cryostat components will
...
\subsubsection{Location } A reasonable criterion for the location of the ITF is within about
a hour drive from Sanford Lab. That criterion yields the following possibilities.
\begin{center}
...
\subsubsection{Management} The management of the ITF should likely be
assigned to one or more DUNE Collaborating Institutions. A possible
choice is SDSM\&T because of its physical proximity, its understanding
of the local infrastructure and relationships and its ability to
provide some specialized effort and specialized facilities that might
be required for the ITF.  Some preliminary discussions with the
SDSM\&T management have already occurred. These discussions should be
ongoing as the parameters for the ITF become more definite.
\begin{center}
...
\fixme{Need an image of the detector showing where the endwalls are
  and where West is. Show 25 rows?}  The SP detector is installed by
...
\fixme{Image of Dual Phase detector.}
...
\begin{itemize}
 ...
 \small
\item Support the weight, both dry and wet \fixme{warm and cold?}, of
  the detector (endwall, top/bottom FC, APA, CPA)
\item Accommodate the roof movement \fixme{THE ROOF DEFORMATIONS NEED TO BE DEFINED}
...
\item Minimize the gaps that develop between APAs during cool down to
  less than 13~mm. \fixme{WHAT IS THE CORRECT VALUE?}  A proposed
  design uses beams that are 6.4m long, which support $\sim$3 APAs.
  This design results in no gap developing between the three APAs on
  the same beam but a 17~mm gap opening up between adjacent APAs on
  separate beams.  There will be a 17~mm gap between every 3$^{rd}$
  and 4$^{th}$ APA.
\item Accommodate installation of the detector. 
\item Define the position of the detector components relative to each other. \fixme{NEED TO DEFINE THIS TOLERANCE}
...
\item The DSS will insure that detector components shall not be less than 400~mm from the membrane flat surface
