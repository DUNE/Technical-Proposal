...
Cable routing schemes for both the TPC electronics and \dword{pds} are actively being developed.  A concept currently under evaluation is to run the cables of the \dword{pds} inside the crossing rib tubes to the central beam tube of the \dword{apa} frames to get to the top.  The \dword{ce} signal and power cables also need to be routed so that the head end of the lower \dword{apa} in the two-\dword{apa} assembly can be reached. The current concept is to route the electronics cables inside the two side beams of the \dword{apa} frames. The right panel of Figure~\ref{fig:apa-cabling} depicts such a cable routing scheme. To fully accommodate the cables from two \dwords{apa}, %it is being considered to modify the \dword{apa} frame design 
