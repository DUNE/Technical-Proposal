...
Given the most recent construction schedule for the Dual-Phase prototype is now likely that the collaboration will forgo beam data taking and focus on  detector performance  assessment with two \dword{crp}s read out and cosmics only. \dword{protodune}-DP will then run with cosmics at a rate going from 20 to 100 Hz from late Fall 2018 to at least April 2019. During six months of operation, with 50\% efficiency, \dword{protodune}-DP is expected to collect about 300 million cosmic triggers at various rates, corresponing to a total data volume of 2.4 PB.


\begin{dunetable}[DoublePhase]
{p{0.5\textwidth}p{0.3\textwidth}}
{sw:np02_data_rate}
{Parameters for a 6 month \dword{protodune}-DP cosmic run at CERN}
Parameter & Value \\
