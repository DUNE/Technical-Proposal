...
Given the most recent construction schedule for \dword{pddp} is now likely that the collaboration will forgo beam data taking and focus on  detector performance  assessment with two \dword{crp}s read out and cosmics only. \dword{pddp} will then run with cosmics at a rate going from 20 to 100 Hz from late fall 2018 to at least April 2019. During six months of operation, with 50\% efficiency, \dword{pddp} is expected to collect about 300 million cosmic triggers at various rates, corresponing to a total data volume of 2.4 PB.
