...
\begin{itemize}
\item {\bf Long-baseline neutrino oscillation studies} - which require a near detector operating in a high rate environment and far detectors in which beam-coincident events are rare but in time with the beam spill and of sufficient energy to be readily recognizable.  Studies discussed in the \dword{daq} section of the Technical Proposal  \ref{sec:fd-daq-ov} indicate that high efficiencies are achievable at an energy threshold of 10 MeV, leading to event rates for beam-initiated  interactions of $\sim 6,400$/year and an uncompressed data volume of around 30 TB/year per 17-kT module. 


Table \ref{tab:daq-data-rates-sw} summarizes the event and data rates after appropriate filtering from the Far detector technical proposal \ref{sec:fd-daq-ov}.
...
\begin{dunetable} [Uncompressed data rates for one SP module.]
  ...
  {tab:daq-data-rates-sw} {Anticipated annual, uncompressed data rates
    for a single SP module from the far detector technical proposal \ref{sec:fd-daq-ov}. The rates for normal (non-SNB triggers)
...
\subsection{Summary}
In summary, data volumes will be dominated by calibration for the far detectors (10 PB/year/module) and by beam and cosmic ray interactions in the near detectors (2-20 PB/year).   With 4 \dword{fd} modules but a factor of 4 compression a total compressed data volume of 12-30 PB/year is anticipated. 
...
\subsubsection{Reconstruction} We have working frameworks for large-scale reconstruction of simulated and real data in place thanks to the \dword{larsoft} effort.  Optimization of algorithms awaits data from protoDUNE. 
...
Parameter & Value \\
   ...
   Requested storage envelope for ProtoDUNE-SP&2 PB at FNAL, 1.5 PB at CERN \\
    \hline
%  \end{tabular}
