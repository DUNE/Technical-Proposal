...
(8) plug sensors onto IDC-4 connectors at a later stage, just before moving corresponding \dword{apa} into its final position. 

For the \single , individual sensors on the top \dword{gp} will have to be integrated with the \dwords{gp}. For each \dshort{cpa} (with its corresponding four \dshort{gp} modules)
...
time. The segments being fed into the \dword{detmodule} will be held at the top
...
hardware interlock on the \dword{hv}, and, in the case of the \dual, to measure the \lar level at the millimeter level as required
for \dual operation.
...
piping, or \dword{dss}. Each enclosure will be attached
...
\fixme{Given that the load... is relatively small}
...
\fixme{to the 4850L}, and stored again before they are used.
