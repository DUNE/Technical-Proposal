...
\begin{itemize}
\item positions of corners of APA or CRP, CPA or cathode, field cages, ground planes (\(\sim\SI{1}{mm}\) scale);
\item relative straightness and alignment of APA/CRP, CPA/cathode, and FC (\(<\sim\SI{1}{mm}\));
...
\item relative straightness and alignment of APA/CRV, CPA/cathode, and FC (\(<\sim\SI{1}{mm}\));
\item gaps between CPA frames (\(<\sim\SI{1}{mm}\));
\item relative position of profiles and endcaps (\(\sim\SI{0.5}{mm}\));
\item sense wires at top of outer wire planes in SP APA (\(\sim\SI{0.5}{mm}\)).
\end{itemize}
...
in liquid argon from \SI{120}{cm} away \cite{Auger:2015xlo}.
