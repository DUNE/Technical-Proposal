...
There are obvious iterfaces with detector consortia since \dword{cisc} will provide full rack monitoring (rack fans, thermometers and rack protection system),
...
On the software side \dword{cisc} will have to define in coordination with other consortia the quantities to be monitored/controlled by slow controls and the corresponding alarms,
archiving and GUIs. 
...
Although this is a \dword{cisc} responsibility, input from \dword{hv} will be crucial.
...
\dword{cisc} data will be stored both locally (in \dword{cisc} database servers in the
\dword{cuc}) and offline (the databases will be replicated back to \fnal)
in a relational database indexed by timestamp.
This will allow bidirectional communications between \dword{daq} and \dword{cisc} by
...
\dword{cisc} also interfaces with the beam and cryogenics group since at least the status of these systems will be monitored.
...
\dword{cisc} will provide the needed monitoring for power distribution units (PDUs). Regarding software interfaces the \dword{swc} group will provide:
...
On the other direction \dword{cisc} will provide the required monitoring and control of \dword{swc} quantities including alarms, archiving and GUIs when applicable. 


\dword{cisc} has profound hardware interfaces with the Calibration Task Force (CTF) \cite{bib:docdb7072}. Indeed, since \dword{cisc} and CTF ports will be multi-purpose to enable deploying various devices,
both systems will need to interact in terms of flange design and sharing space around the ports. Also, \dword{cisc} might use calibration ports to extract cables from \dword{cisc} devices. 
At the software level \dword{cisc} will be responsible for calibration device monitoring (and control to the extent needed) and will 
...
\dword{cisc} indirectly interfaces to physics through the CFT devices. One specific need for physics will be to extract
