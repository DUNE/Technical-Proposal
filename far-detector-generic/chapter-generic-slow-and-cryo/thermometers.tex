...
have been previously used in the \dword{35t} and \dword{pdsp} detector,
...
the capsule is introduced in a \threed printed polylactic acid (PLA) box of size \(9.5\times9.5\times\SI{19}{cm^3}\), with two concentric independent volumes of \lar
...
Then the capsule is immersed in  \lar such that it penetrates
...
The baseline design for the mechanics of the \single system consists of two stainless strings anchored at top and bottom corners of the cryostat
...
on the sensor side; on the other side, it is directly soldered into the inner pins of male SUBD-25 connectors on the flanges. The CF63 side ports on the \dword{dss}/cryogenic ports are 
%which will be 
...
The dynamic temperature monitor is a vertical array of high precision temperature sensors with the goal of measuring vertical temperature gradient with precision of few \si{mK}. The design of the system is driven by two factors:
\begin{itemize}
\item
...
%%anne to here saturday 4/27
 ...
 In order to address concerns related to possible differences in the sensor readings prior to %installation 
...
\item A high-resolution and accuracy voltage signal readout module based on National Instruments~\footnote{National Instruments\texttrademark{}, \url{http://www.ni.com/en-us.html/}.} NI9238, which has \SI{24}{bit} resolution over a \SI{1}{V} range.
  ...
  through the standard OPC UA protocol. The Ethernet \dword{daq} also includes the multiplexing logic.
\end{itemize}
