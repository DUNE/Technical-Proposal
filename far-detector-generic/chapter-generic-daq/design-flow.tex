...
\dword{ringbuffer} for the \dword{daqfrag} which resides in the
...
the \dword{ringbuffer} must arrive within this buffer time.
...
multiple \dwords{ringbuffer} across the DAQ.
...
that the \dword{ringbuffer} no longer contains the data then an error
...
In the alternative design the same \dword{ringbuffer} provides the
