...
\dwords{detmodule}, only data from TPC collection channels ($\frac{3}{8}$ of SP and all of DP channels) will feed
...
data from the \dword{sp} \dword{detmodule} will fill about half of the
...
the many \dwords{datafrag} in each trigger which will be consistent
...
automated process. 
If not rejected it is then sent out of the \dword{daq} to permanent offline
storage.
...
aggregated ({\it i.e.}, as they would be for a \dword{snb}).
...
({\it e.g.}, \SI{10}{\s}), decide to generate a \dword{snb} trigger command.
...
created. 
Information on \dwords{trigcandidate} will also be kept, whether or not
