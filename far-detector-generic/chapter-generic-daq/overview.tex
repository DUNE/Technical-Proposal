...
\begin{dunefigure}[DAQ Overview]{fig:daq-overview}
  {The high-level, \textbf{nominal} design for the DUNE FD DAQ in
    ...
    \dword{daqfrag} front-end and the trigger processing and event
...
% \includegraphics[width=0.8\textwidth]{high-level-daq.pdf}%
  \includegraphics[width=0.8\textwidth]{daq-nom-hl.pdf}%
\end{dunefigure}
...
% \includegraphics[width=0.8\textwidth]{high-level-daq-alt.pdf}%
  \includegraphics[width=0.8\textwidth]{daq-alt-hl.pdf}%
\end{dunefigure}
...
which will service both SP and DP \dwords{detmodule} and be reasonably
...
For both, the diagrams are drawn centered on one \dword{daqfrag}
front-end which is a portion of the entire \dword{daqpart} servicing a
\dword{detmodule} which has one \dword{fec} accepting about
...
\begin{itemize}
\item Accept TPC and PDS data from the \dwords{detunit} associated with the \dword{daqfrag}.
...
(aka the DAQ instance) servicing a portion of one \dword{detmodule}
...
\item Exchange \dwords{trigcandidate} with the \dword{gtl}
...
\item The \dword{eb} interprets and executes the command by making
  data selection requests to referenced \dwords{daqfrag}.
\item The \dword{eb} accepts the returned \dword{datafrag} from each
  \dword{daqfrag} and combines them into a \dword{rawevent}.
\item Write the result to the \dword{diskbuffer} which is the boundary
  shared with DUNE offline computing.
\end{itemize}

The nominal and the alternative DAQ designs differ largely in where
...
return the full \dword{sbn} data immediately, will stream it to local
\dword{ssd} storage. 
From that storage, the data is sent to the \dword{eb} as a low
priority (ie, also ``trickled out'').
Since the \dword{mtl} assures no overlapping commands the the buffer
nodes may service subsequent requests from post dump data that will be
in the RAM buffer.
And, since each trigger command is handled by an individual \dword{eb}
...
including some of which is specific to a given \dword{detmodule}, is
...
\item Front-end computing for hosting of \dword{daqdr}, \dword{daqbuf} and \dword{daqds}.
...
\item DAQ data handling software including receiving and building of
  events
\item The \dword{om} of DAQ performance and data content.
