...
\metainfo{DP/SP shared.  Georgia Karagiorgi and Dave Newbold. 2 Pages - largely
  ...
  Focus on describing to HEP but non-\dword{daq} expert. 
...
The current generation of \lartpc \dword{daq}, such as used in
...
For example, the \dword{spmod} has \dwords{apa} each
providing data from a number of \dwords{wib} and the \dword{dpmod} has
...
\begin{dunefigure}[\dword{daq} overview]{fig:daq-overview-alt}
  {The high-level, \textit{alternative} design for the DUNE \dword{fd}\dword{fd} \dword{daq} in
    terms of data (solid) and trigger (dashed) flows between one
    \dword{daqfrag} \dword{fe} and the trigger processing and event
    building back end for one \dword{daqpart}. 
    Line thickness indicates relative bandwidth requirements.
...
The high-level requirements for the DUNE \dword{fd} \dword{daq} are provided in
\cite{daq:reqs}.
The most critical requirements %in consideration of the overall design
...
\begin{dunetable} [Pre-trigger data rates from the  \dword{fd} TPCs and
  ...
  \colhline
  TPC unit & \dword{apa} & \dword{cro} crate \\
  unit multiplicity & \num{150} & \num{240} \\
...
cryostat. 
Studies using data from the LBNE \dword{35t} prototype and early \microboone
