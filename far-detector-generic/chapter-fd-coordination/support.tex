...
the EB and/or Resource Coordinator (RC).
\begin{figure}[htb]
  \begin{center}
...
\begin{itemize}
  \item Assure that each consortium has a well defined and complete
    ...
    through Common Fund or flag missing scope to the EB and RC. In
  ...
  \item Develop an overall integrated master project schedule (IMS)
    that includes reasonable production schedules from each consortia,
    testing plans and a well developed installation schedule. Monitor
...
design (\dword{pdr} and \dword{fdr}), production (\dword{prr} and
...
engineering team. TC will maintain a calendar of \dword{dune} reviews.
...
may work under the \dword{lbnf}/\dword{dune} \dword{qap} and develop
Manufacturing/QC Plans, procedures and documentation specific to their
work scope.  The \dword{dune} Technical Coordinator and consortia
...
and verify work that affects quality.  The \dword{dune} consortia
...
described in the \dword{lbnf}/\dword{dune} \dword{ieshp}. This plan is
maintained by the \dword{lbnf}/\dword{dune} ESH Manager, who reports
to the \dword{lbnf} Project Manager and the TC. The ESH is responsible
...
reviews will be conducted as part of the \dword{prr} and \dword{ppr} process described
