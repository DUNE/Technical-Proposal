...
\begin{itemize}
  \item {\bf Transport Buffer:} \dword{itf} needs to provide storage capacity 
    ...
    laboratory.
  \item {\bf Longer-term Storage:}  \dword{itf} needs to provide longer-term 
  ...
  \item {\bf Re-packaging} \dword{itf} needs to have the ability to re-package 
  ...
  \item {\bf Component Fabrication:}  The fabrication of some components 
    ...
    Lab at the \dword{itf}, taking advantage of undergraduate science and engineering 
  ...
  \item {\bf Inspection, Testing and Repair:} In cases where components 
    are re-packaged for transport to the undergound area, \dword{itf} support 
    ...
    integrated at the \dword{itf} would require additional testing prior to being 
    re-packaged.  The \dword{itf} could provide facilities needed to repair some 
...
facilities, in contrast to the \dword{itf}, are relatively straight-forward and 
...
operation of the \dword{lbnf} surface facilities and the \dword{dune} \dword{itf} 
...
and Rapid City.  The \dword{lbnf} surface facilities, which are not expected 
to have significant auxilliary functionality, would logically be located as 
near to the Sanford Lab site as possible.  In the case of the \dword{dune} 
\dword{itf}, however, locating the facility in Rapid City to take advantage of 
...
incorporating the \dword{dune} \dword{itf} into an overall regional development 
program.     

\subsection{Requirements}

The leadership teams associated with the \dword{dune} institutional consortia
taking responsibility for the different far detector subsystems have provided 
input to TC on which potential \dword{itf} functions and services would be applicable 
...
these functions and services to be provided for their subsystems at the \dword{itf}.
An attempt at capturing preliminary, global requirements for the \dword{itf} based 
...
\begin{itemize}
  ...
  \item Access to shared office space for up to 30 collaborators contributing 
    to the activities taking place at the \dword{itf}.  Assistance for identifying 
    temporary housing in the vicinity of the \dword{itf} for the visiting collaborators.        
\end{itemize}

\subsection{Management}

Overall management of the \dword{itf} is envisioned to be reponsibility of one or 
more of the collaborating institutions on \dword{dune}.  If the \dword{itf} is located 
...
and facilities that would benefit planned \dword{itf} activities.  Intial discussions 
with SDSM\&T representatives have taken place, in which they have expressed a 
clear interest for hosting the \dword{itf}.  Additional discussions will be needed to 
understand the details of the \dword{itf} management structure with in the context of 
...
Integration and Test Facility, the \dword{itf} is the location at which
...
the \dword{itf} planning.  A critical requirement for the project is that the
