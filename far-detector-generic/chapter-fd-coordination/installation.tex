...
\dword{uit} will make the installation plan that describes how the
...
delivered by \dword{lbnf}), scissor lifts, areal lifts, and the common
...
\dword{lbnf}/SURF from the time the components reach the Ross
...
of contact if the \dword{lbnf}/SURF team has questions or difficulties
with the underground transport.  The \dword{uit} receives the
materials from \dword{lbnf}/SURF at the entrance to the \dword{dune}
...
\dword{uit} has focused initially on the \dword{sp} detector as the
...
\caption{Left: Image of the \dword{dp} \dwords{crp} being installed in
  the detector, showing the connection from the \dword{crp} to the
...
from the inside using the roof access holes for access. In parallel to this the
