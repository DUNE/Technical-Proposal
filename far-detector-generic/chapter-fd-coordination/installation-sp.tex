...
to the Technical Coordinator. The surface and undergound efforts will be referred to as
...
coordinating with the \dword{lbnf} logistics effort and acting as the
interface to \dword{lbnf} logistics for planning transport of all detector
equipment underground. One challange DUNE faces is that there is very limited above ground space near SURF, so a nearby facility will be needed to receive, store and ship materials to the Ross Shaft in a just in time fashon. The Surface Team will receive equipment from the consortia
and plan the transport to the Ross shaft with the \dword{lbnf} logistics
...
SURF site. Consorita may request more local storage but this information is still being gathered. The logistics effort will need warehouse space with an associated
...
Underground installation is in general responsible for coordinating and supporting the installation of
the detectors and providing necessary infrastructure for the experiment. While the individual consortia are responsible for the installation of their own detector equipment it is essential that the detector installations be planned as a whole and that a single group coordinates the installation and adapts the plans throught the installation process. The UIT has the responsiblility for the overall coordination of the installations. In conjunction with the consortium the UIT will make the installation plan that describes how the detectors are to be installed. The installaiton plan is used to define the spaces and infrastructure that will be needed to install the detectors. The installation plan will also be used to define the interfaces between the Installation group and the consortia deliverables.  The installation scope includes the
...
fixtures as required to install thier detector components. In the case of the single-phase detector the installaiton goup will
also provide the detector support system (DSS) which supports the detector and is needed to bring equipment
into the cryostat.

In addition to the installaiton specific infrastructure the installaiton goup will also be resopnsible for general detector
...
maintain the \dword{dune} installation plan and the installation
schedule. The \dword{dune} installation plan will describe the
...
coordinate the \dword{dune} installation effort and adapt the schedule as
...
\dword{lbnf}), scissor lifts, areal lifts, and the common work platforms
...
The UIT acts as the primary point of contact with \dword{lbnf}/SURF from the
...
point of contact if the \dword{lbnf}/SURF team has questions or difficulties
with the underground transport.  The UIT receives the materials from
\dword{lbnf}/SURF at the entrance to the \dword{dune} excavations. The UIT then
...
it forms.  \fixme{images are too small}
\begin{figure}[!htb]
\begin{center}
...
The current installation plan is desscribed. \dword{dune} will take
...
\end{center}
\end{figure}
The the \dwords{apa} and \dwords{cpa} with top/bottom FC panels are
...
(\dword{dp}) with photon detection. The individual detector elements
of the \dword{dp} are smaller than the \dword{sp} detector so the
installation infrastructure is simpler. The \dword{dp} detector will
need a cleanroom and cranes capable of moving the equipment to the
cleanroom entrance. Much of the work for the \dword{dp} installation
will be performed inside the cryostat. Like the \dword{sp} detector
...
being installed the \dword{dp} chimneys for the electronics along with
the DPPD and CRP instrumentation feedthrus can be installed. The
\dword{dp} detector requires a much smaller cleanroom which is
...
the full detector is foreseen. The \dword{dp} installation sequence is
